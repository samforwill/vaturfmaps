...
if len(filtered_metrics) > 0:
    ...
    edit_df = filtered_metrics[['van_precinct_id', 'van_precinct_name', 'county_name', 
                                'Current Region', 'Current Turf', 'voters', 'supporters']].copy()
    ...
    column_config = {
        "van_precinct_id": st.column_config.TextColumn("Precinct ID", disabled=True),
        "van_precinct_name": st.column_config.TextColumn("Precinct Name", disabled=True),
...
with st.expander("Debug Info"):
    ...
    st.write(f"Changed precincts: {st.session_state.changed_precincts}")